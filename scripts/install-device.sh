...
cd "$(dirname "$0")/.."
APP="DerivedDataDevice/Build/Products/Debug-iphoneos/FishyMotion.app"
DEVICE="${1:-24FD2D6E-6EE2-59CE-90EC-39B48B6FF171}"

if [[ ! -d "$APP" ]]; then
  xcodebuild -project FishyMotion.xcodeproj \
    -scheme FishyMotion \
    -configuration Debug \
...
xcrun devicectl device process launch --device "$DEVICE" com.sergiiziborov.fishymotion
